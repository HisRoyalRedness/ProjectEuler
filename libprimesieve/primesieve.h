 ...
 * 
 *  Amended to export functions - Keith Fletcher - 13 May 2017
 *
 ...
 */
__declspec(dllexport) void* primesieve_generate_primes(uint64_t start, uint64_t stop, size_t* size, int type);

/**
 * Get an array with the first n primes >= start.
 * @param type  The type of the primes to generate, e.g. INT_PRIMES.
 */
__declspec(dllexport) void* primesieve_generate_n_primes(uint64_t n, uint64_t start, int type);
 ...
 */
__declspec(dllexport) uint64_t primesieve_nth_prime(int64_t n, uint64_t start);
 ...
 * number of threads.
 */
__declspec(dllexport) uint64_t primesieve_count_primes(uint64_t start, uint64_t stop);
 ...
 * number of threads.
 */
__declspec(dllexport) uint64_t primesieve_count_twins(uint64_t start, uint64_t stop);
 ...
 * number of threads.
 */
__declspec(dllexport) uint64_t primesieve_count_triplets(uint64_t start, uint64_t stop);
 ...
 * number of threads.
 */
__declspec(dllexport) uint64_t primesieve_count_quadruplets(uint64_t start, uint64_t stop);
 ...
 * number of threads.
 */
__declspec(dllexport) uint64_t primesieve_count_quintuplets(uint64_t start, uint64_t stop);
 ...
 * number of threads.
 */
__declspec(dllexport) uint64_t primesieve_count_sextuplets(uint64_t start, uint64_t stop);
 ...
 */
__declspec(dllexport) uint64_t primesieve_get_max_stop();

/** Get the current set sieve size in kilobytes */
__declspec(dllexport) int primesieve_get_sieve_size();

/** Get the current set number of threads */
__declspec(dllexport) int primesieve_get_num_threads();
 ...
 * functions.
 */
__declspec(dllexport) void primesieve_free(void* primes);

/** Get the primesieve version number, in the form “i.j” */
__declspec(dllexport) const char* primesieve_version();
